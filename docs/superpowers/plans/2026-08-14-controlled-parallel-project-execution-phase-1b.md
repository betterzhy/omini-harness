...
Task 8 Final Gate Remediation Round 2 Authority + WriteSet migration (fixed base/parent `766281106fe70221222b8b1d8304795dc0343f79`): the fresh xhigh review closed the protected-action and complete-review-set findings but returned `NO-GO / P0=0 / P1=1 / P2=0` because pathname-based Git execution remained vulnerable to an administration-root swap/read/restore between successful pre- and post-identity checks. The exact authorized six-path Round 2 WriteSet is `src/evolution_harness/controlled_coordinator.py`, `src/evolution_harness/controlled_write_guard.py`, `tests/test_controlled_coordinator_lifecycle.py`, `tests/test_controlled_coordinator_acquire.py`, `tests/test_controlled_write_guard.py`, and this formal plan. Source HEAD, lane HEAD, Candidate parent/tree, and guarded Git inventory must consume the already-opened physical repository view, or derive the same evidence directly from descriptor-anchored refs and objects; comparing mutable pathnames before and after a Git subprocess is not authority. The implementation must retain the absolute pinned Git executable, sealed environment, no-replace and no-alternates constraints, and add a real linked-worktree swap/read/restore negative that proves transient alternate administration or object state cannot be observed as authoritative evidence. If the host cannot provide a descriptor-consuming inventory boundary, execution must fail closed before the target command rather than silently retain the pathname TOCTOU.

Task 8 Final Gate Remediation Round 3 Authority + WriteSet migration (fixed base/parent `7bf290f13224d60d62ea06d854fc049863259bdf`): read-only diff inspection found that the Round 2 linked-source test hooked a Git subprocess that descriptor-based HEAD resolution no longer invokes, so its green result did not prove that the intended transient administration swap actually occurred. The same six-path WriteSet remains authorized. Every transient negative must assert that its exchange hook executed around the exact descriptor/ref/object read being tested; linked-worktree guarded execution may instead prove that it returns `LANE_INVENTORY_UNAVAILABLE` before any target or inventory subprocess starts. Object bytes remain authoritative only after reading through the held objects descriptor and recomputing the explicit object id; mutable Git administration/config path input may not choose a ref, substitute object bytes, or change an accepted Candidate/Parent/Tree.

Task 8 Final Gate Remediation Round 4 Authority + WriteSet migration (fixed base/parent `19b830cfbb19bc0d8f3b0193107cdbd784d13bd5`): the fresh fixed-candidate gate returned `NO-GO / P0=0 / P1=2 / P2=0`. The exact authorized eight-path WriteSet is `src/evolution_harness/authority.py`, `src/evolution_harness/controlled_write_guard.py`, `tests/test_authority_engine.py`, `tests/test_controlled_coordinator_acquire.py`, `tests/test_controlled_coordinator_lifecycle.py`, `tests/test_controlled_write_guard.py`, `tests/test_controlled_recovery.py`, and this formal plan. Every live Acquire and Transition Authority Snapshot must bind source HEAD, commit tree, committed authority blobs, and working authority bytes to the same descriptor/object view; caller PATH, Git environment, replace refs, alternate object configuration, and ambient `hash-object` may not report uncommitted authority bytes as `CLEAN_FOR_AUTHORITY_SET`. Complete physical inventory must include pre-existing nonignored empty leaf directories so guard quarantine and public recovery observation see the same persistent breach set. Add full acquire/transition ambient-Git and replace negatives plus pre-existing empty-directory guard/observation negatives before implementation; keep linked-worktree guarded execution as an explicit pre-launch fail-closed residual.

Task 8 Final Gate Remediation Round 5 Authority + WriteSet migration (fixed base/parent `e9b3ddfd6f12a0ed54897f964d2ffcd04906d6c9`): the Round 4 fixed-candidate gate returned `NO-GO / P0=0 / P1=1 / P2=1` on test evidence only; its independent probes found no production defect. The exact authorized three-path WriteSet is `tests/test_controlled_coordinator_acquire.py`, `tests/test_controlled_recovery.py`, and this formal plan. The Acquire ambient-Git/replace negative must construct the clean command first, then inject the uncommitted Authority and hostile ambient Git view, prove the real live-rebuild boundary executed, exact-match `LIVE_AUTHORITY_SNAPSHOT_MISMATCH`, prove ambient Git was not invoked, and prove no lease or receipt was initialized. The pre-existing empty-directory public observation negative must first prove an incomplete caller set is rejected, then pass the complete set through `observe_lane_writes` and prove project quarantine plus durable recovery evidence and receipt preserve that directory. No production file is authorized in Round 5.
