...
- only an exact pre-HG3 Promotion entry listed in the approved legacy cutover may remain a Release with partial lineage; a missing post-cutover Plan, binding, Candidate transition, or Receipt fails closed;
...
- `PromotionPlanReference` binds Schema version, Promotion Plan ID, transaction ID, content-addressed owner-only snapshot identity, raw plan digest, canonical plan digest, and `promotionPlanDigest`. It intentionally excludes every journal-entry and terminal-Receipt reference, so `BUNDLE_ADMITTED` can bind the Plan without a digest cycle. A Plan is eligible only when a valid Promotion journal generation admits this exact reference.
- `PromotionJournalEntryReference` uses the same sequence, prior-digest, prefix-digest, writer-time, lock, immutable-prefix, atomic-replace, reread, and compare-and-swap rules as `ExecutionJournalEntryReference`, but binds one Promotion transaction journal and the closed event type `BUNDLE_ADMITTED | ARTIFACTS_STAGED | CANDIDATE_STATUS_APPLY_INTENT | CANDIDATE_STATUS_APPLIED`. The two journal kinds cannot be substituted. Its event digest excludes its own derived entry/prefix digests by the same formula. Every event in every generation repeats one byte-identical `PromotionPlanReference` and `promotionPlanDigest`. Sequence 1 is always `BUNDLE_ADMITTED` and carries `previousGenerationTerminalReference`: generation 1 uses `NOT_AVAILABLE/FIRST_GENERATION`; a recovery generation uses PRESENT with the exact terminal-prefix entry of the immediately preceding generation, increments generation by one, and keeps the same transaction/Plan. Generations form one nonbranching chain. A recovery generation must then emit its own `ARTIFACTS_STAGED` and `CANDIDATE_STATUS_APPLY_INTENT`; it cannot consume staged or intent entry references from another generation. At most one generation may reach `CANDIDATE_STATUS_APPLIED` or a terminal Receipt.
...
4. Atomically persist the `CandidateBundleReference` plus either those frozen bytes or content-addressed immutable blob identities, materialize the immutable `growth-promotion-plan/v1`, and append `BUNDLE_ADMITTED` with that exact `PromotionPlanReference` before staging Capability, ledger, or binding data. The Plan stores the deterministic planned post-Promotion `candidate.yaml` bytes whose parsed object differs only by `promotionStatus: INTEGRATED`.
5. Stage exclusively from the frozen journal snapshot, never by reopening live Candidate paths. Re-read staged bytes and require the same asset/content canonical values and proposed content hash before publication.
6. Apply the Candidate status only through the journaled compare-and-swap protocol in Section 7.3.5. No terminal Promotion Receipt may exist until the live file has moved from the admitted raw digest to the planned integrated digest, has been durably reread, and has an exact `CANDIDATE_STATUS_APPLIED` entry.

A path change after journal admission therefore cannot alter promoted bytes. Interruption or disagreement leaves only recoverable staging or a separately classified pending Candidate-status transition, publishes no terminal Receipt, and cannot expose a Release. The post-CAS `promotionStatus` update is a different immutable snapshot and never changes the admitted pre-Promotion reference. Candidate ID, target Capability, proposed asset ID/version, contentFile contract, and computed content hash must be internally consistent before any Experiment, binding, Receipt, or Release may consume the bundle. A live `INTEGRATED` label without the matching terminal Receipt is `INTEGRATION_PENDING_RECEIPT | RECOVERY_REQUIRED`, never successful Promotion evidence.
...
Current historical `candidate/v1`, `eval-result/v1`, and `promotion-ledger/v1` entries are never retroactively rewritten. The new path performs only the existing forward `AUTHORIZED -> INTEGRATED` Candidate transition under the strict transaction below. Only a Promotion entry frozen into the approved legacy cutover contract may remain a ledger-backed Release with partial lineage; a post-cutover entry missing its required Plan, binding, Candidate transition, or Receipt is invalid. HG3 must add the trial-scoped execution and Promotion plan/transition/receipt path before any new Experiment-backed claim is possible.

#### 7.3.4 `growth-promotion-plan/v1`

This immutable, content-addressed record freezes the complete authorized Promotion write plan before any governed artifact is staged. Required fields are:
...
plannedCapability
plannedLedgerEntry
plannedLineageBindingPayload
plannedIntegratedCandidateSnapshotReference
plannedIntegratedCandidateRawDigest
promotionPlanDigest
```

`schemaVersion` is `growth-promotion-plan/v1`; command, Authority, and Candidate use their shared exact reference types. `plannedCapability` contains the exact destination, Capability ID/version, asset raw/canonical digest, content raw/normalized digest, and computed content hash. `plannedLedgerEntry` contains the complete canonical entry, fixed `authorizedAt`, unique tuple, entry digest, and planned enclosing-ledger digest. `plannedLineageBindingPayload` contains every future binding field and reference except `promotionPlanReference`, `promotionPlanDigest`, the binding's outer `AssetRevisionReference`, and its derived record digest; excluding both Plan fields is required to prevent a Plan/Binding digest cycle. The planned integrated Candidate reference binds the deterministic postimage already frozen at bundle admission.

`promotionPlanDigest` is `sha256:` of canonical Plan bytes excluding only `promotionPlanDigest`. It therefore binds transaction, command/idempotency, Authority, the complete pre-Promotion Candidate bundle, proposed content hash, promotion path, lineage intent, exact planned Capability/ledger/binding payloads, and planned Candidate postimage. `PromotionPlanReference` binds both these canonical bytes and raw serialized bytes. Capability, ledger, binding, or Candidate bytes that do not recompute to this Plan are not the authorized transaction even when their logical IDs match.

#### 7.3.5 `growth-candidate-integration-transition/v1`

This immutable transition proves that the mutable legacy `candidate/v1.promotionStatus` was compare-and-swap applied inside the same Promotion transaction before its terminal Receipt. Required fields are:
...
promotionCommandReference
authorityDecisionReference
prePromotionCandidateReference
expectedCandidateRawDigest
plannedIntegratedCandidateSnapshotReference
plannedIntegratedCandidateRawDigest
integratedCandidateReference
integratedCandidateRawDigest
candidateStatusAppliedEntryReference
...
`schemaVersion` is `growth-candidate-integration-transition/v1`. `promotionPlanReference` is a `PromotionPlanReference`, and the repeated `promotionPlanDigest` must equal it. `promotionCommandReference` is a `CommandReference`; `authorityDecisionReference` is the exact `AuthorityDecisionReference` frozen by that command and Plan; `prePromotionCandidateReference` is the Plan's admitted `CandidateBundleReference`; the planned and integrated Candidate references are `AssetRevisionReference` values to owner-only journal snapshots of the exact before-publication plan and post-CAS reread bytes; and `candidateStatusAppliedEntryReference` is a `PromotionJournalEntryReference` constrained to `CANDIDATE_STATUS_APPLIED`. Transaction ID, Plan, Candidate ID, command, Authority, and planned/post digests must equal that same-generation journal event, and `appliedAt` must equal its writer-owned `recordedAt`. The expected raw digest must equal the admitted `candidateRawDigest`; both planned and integrated raw digests must equal each other and use `sha256:<64 lowercase hex>`. Parsed planned/integrated `candidate.yaml` must equal the admitted Candidate object field-for-field except for exactly `promotionStatus: AUTHORIZED -> INTEGRATED`; `authorityDecision` remains `APPROVE`. Candidate ID, operation, target, source Experiences, evidence, Eval requirements, transfer evidence, proposed asset/content references, and every other field are immutable.

The Promotion journal and filesystem order is closed:

```text
BUNDLE_ADMITTED
< ARTIFACTS_STAGED
< CANDIDATE_STATUS_APPLY_INTENT
< candidate.yaml compare-and-swap + durable reread
< CANDIDATE_STATUS_APPLIED
< immutable Candidate Integration Transition
< terminal Promotion Receipt
```

Every event in the sequence repeats the exact `PromotionPlanReference` and `promotionPlanDigest`. `ARTIFACTS_STAGED` binds the exact staged Capability, ledger entry, lineage binding, and planned integrated Candidate bytes and proves that each recomputes to that Plan. Immediately before the Candidate write, the operation proves that the live Candidate is the exact admitted raw bytes with `AUTHORIZED/APPROVE` and that the authoritative decision source Revision/digest still matches, then durably appends same-generation `CANDIDATE_STATUS_APPLY_INTENT` with that validation, the expected/prepared digests, and the frozen decision. The intent fixes transaction authority; later authority changes are separate lifecycle events. The operation then performs an anchored, no-follow, atomic compare-and-swap from `expectedCandidateRawDigest` to the planned bytes, fsyncs, rereads, and appends same-generation `CANDIDATE_STATUS_APPLIED` with the exact post-write reference and Plan digest. A mismatch before the intent or CAS is `CANDIDATE_STATUS_CONFLICT`: no applied entry or terminal Receipt is written, `SUCCEEDED` is forbidden, and staged artifacts remain non-authoritative recovery evidence.

Recovery is deterministic and never guesses. Before an apply intent, recovery may resume only after the live preimage and Authority Decision still revalidate. After an intent but before the Candidate CAS, a live preimage requires renewed Authority validation in a successor journal generation; that generation must link the preceding terminal prefix, repeat the same immutable Plan, and emit its own complete `BUNDLE_ADMITTED -> ARTIFACTS_STAGED -> CANDIDATE_STATUS_APPLY_INTENT` prefix before retry. No event or staged reference crosses generations, and the old intent is never silently replayed. If interruption occurs after the filesystem CAS but before its applied entry, recovery stays in that generation: the durable intent already contains the exact Plan, preimage, planned postimage, and frozen Authority Decision, so a live postimage permits the missing applied entry and transition to be completed; any third digest or missing durability is `PROMOTION_OUTCOME_UNCERTAIN` and requires explicit recovery. After `CANDIDATE_STATUS_APPLIED`, the recorded Authority snapshot is the transaction authority; a later authority change is a separate lifecycle decision and cannot rewrite the in-flight history. Recovery may publish the one deterministic terminal Receipt only after all same-generation staged artifacts, the applied entry, the transition, and the shared Plan revalidate. It never automatically rolls Candidate status back or deletes evidence.

A visible `candidate/v1` value of `INTEGRATED` without the exact transition and terminal Receipt is therefore only `INTEGRATION_PENDING_RECEIPT | RECOVERY_REQUIRED`. It never creates a Release or causes the command to report success. A COMMITTED Receipt must reference exactly one valid transition and thereby proves that Candidate status had already been durably applied; later live Candidate drift is reported as divergence without invalidating the immutable historical transition.

#### 7.3.6 `promotion-receipt/v1`
...
canonicalCandidateBundleDigest
proposedCapabilityContentHash
candidateIntegrationTransitionReference
capabilityReference
promotionPath
lineageIntentDigest
...
`schemaVersion` is `promotion-receipt/v1`; `commitState` is exactly `COMMITTED`. `promotionPlanReference` is a `PromotionPlanReference`, and the repeated `promotionPlanDigest` must equal it, the binding, transition, and every journal event in the successful generation. `promotionCommandReference` is a `CommandReference`; `candidateReference` is the pre-Promotion `CandidateBundleReference`; the repeated `canonicalCandidateBundleDigest` and `proposedCapabilityContentHash` must byte-equal that reference, Plan, and lineage binding. `candidateIntegrationTransitionReference`, Capability, and lineage-binding references are exact `AssetRevisionReference` values; `authorityDecisionReference` is an `AuthorityDecisionReference`; the ledger reference is a `LedgerEntryReference`. The transition must carry the same transaction, Plan, command, authority, pre-Promotion Candidate, and exact status-applied entry, and must lead to `INTEGRATED`. `promotionPath` is `EXPERIMENT_BACKED | CANDIDATE_EVAL_BACKED` and must equal the referenced binding kind and Plan. The Receipt repeats the command's `idempotencyKey`, Plan digest, Candidate bundle digest, proposed Capability content hash, and lineage-intent digest and binds the exact transition digest, binding digest, canonical Capability ID/version/content hash and asset digest, ledger tuple and entry digest, transaction ID, authoritative decision, commit time, and bounded evidence references. All planned/staged references must recompute to the Plan. Its canonical digest excludes no referenced source bytes and excludes only its own outer `AssetRevisionReference`, which is created after the Receipt bytes exist.

The journal enforces uniqueness by both `(promotionCommandReference.commandId, idempotencyKey)` and the `(capabilityId, capabilityVersion)` ledger tuple. Exact replay of the same key and PayloadHash returns the same Receipt bytes. The same key with another payload is `IDEMPOTENCY_CONFLICT`; a second successful Receipt for either unique key is invalid. Capability, ledger entry, lineage binding, Candidate status CAS/applied entry, and Candidate Integration Transition are durably validated in that order; this Receipt is published last and reread. Only then may the operation return `SUCCEEDED`.

Loss of the response or uncertain directory durability after staging or Candidate CAS never authorizes replay as a new Promotion. Receipt lookup uses Command ID and IdempotencyKey: one exact valid COMMITTED Receipt with its exact Candidate Integration Transition returns that Receipt; no Receipt with recoverable staged or status-applied journal state requires explicit recovery; disagreement, multiplicity, a third Candidate digest, changed pre-CAS Authority, or unknowable durability returns `PROMOTION_OUTCOME_UNCERTAIN` and fails closed. Generic `UNKNOWN_OUTCOME`, an isolated `INTEGRATED` Candidate label, or recovery records never project as a Promotion Receipt or Release.

#### 7.3.7 `growth-legacy-promotion-cutover/v1`
...
HG3 freezes this manifest under an explicit user-authorized fixed candidate before the new Promotion path is enabled. Only listed entry digests may select `HISTORICAL_PROMOTION_PARTIAL`. Any new or changed `PROMOTED` entry after the cutover must have one valid Promotion Plan, lineage binding, Candidate Integration Transition, and terminal `promotion-receipt/v1`; otherwise it emits no Release and makes the projection Gate `NO_GO`. The cutover never fabricates Candidate, Eval, Experiment, Trial, Plan, transition, or Receipt facts for historical entries.
...
`capabilityReference` and every source Experiment, Trial, Eval Result, and Expectation array element have type `AssetRevisionReference`; every source Candidate element has type `CandidateBundleReference`; each array rejects duplicate identity/digest keys. `ledgerEntryReference` has type `LedgerEntryReference`. `promotionPlanReference` has type `Availability<PromotionPlanReference>`. `lineageBindingReference`, `promotionReceiptReference`, `candidateIntegrationTransitionReference`, `bootstrapBaselineReference`, `bootstrapContentBindingReference`, and `legacyPromotionCutoverReference` each have type `Availability<AssetRevisionReference>` with closed `PRESENT` and `NOT_AVAILABLE` `oneOf` branches. `EXPERIMENT_BACKED` requires PRESENT Plan, lineage, Candidate transition, and Receipt references and NOT_AVAILABLE bootstrap/content-binding/cutover references, with one byte-identical Plan across the binding/transition/Receipt, a valid binding plus terminal Receipt whose bound lineage intent consumed a PASSED Experiment and its exact same-Trial Eval receipts, and a transition proving the pre-Receipt Candidate status CAS. `CANDIDATE_EVAL_BACKED` has the same reference availability but consumes a valid non-Experiment binding, Plan, Candidate transition, and terminal Receipt. Historical and bootstrap paths require both `promotionPlanReference` and `candidateIntegrationTransitionReference` NOT_AVAILABLE with path-specific reasons. A PASSED Experiment, Plan, lineage binding, Candidate transition, or Promotion Receipt without the canonical asset and existing ledger entry never creates a Release.

`BOOTSTRAP_AUTHORIZED` is selected only when all of the following exact sources agree: the ledger entry has `authorization: BOOTSTRAP_AUTHORIZED` and `authorityDecision: CLOSED_BASELINE_BOOTSTRAP`; its tuple and content hash match the canonical Capability; the tuple appears exactly once in `authorizedSeeds[]` of the immutable baseline referenced by PRESENT `bootstrapBaselineReference`; the content binding's own `bootstrapBaselineReference` is byte-identical to that Release reference across kind, ID, source Revision, and digest; and the exact Capability/ledger/content hash tuple appears exactly once in the immutable companion referenced by PRESENT `bootstrapContentBindingReference`. Its Plan, lineage, Candidate-transition, Promotion Receipt, and legacy-cutover references are `NOT_AVAILABLE` with path-specific reasons. A claimed seed absent from either source, a cross-baseline splice, a duplicate, a changed Capability/ledger pair, or any disagreement is invalid, emits no Release, and makes the projection Gate `NO_GO`.

Promotion-path selection is mutually exclusive and ordered: a validated bootstrap tuple selects `BOOTSTRAP_AUTHORIZED`; a `PROMOTED` ledger entry with one valid Plan, Experiment binding, Candidate transition, and Receipt selects `EXPERIMENT_BACKED`; a `PROMOTED` entry with one valid Plan, Candidate/Eval binding, Candidate transition, and Receipt selects `CANDIDATE_EVAL_BACKED`; a `PROMOTED` entry whose exact digest is PRESENT exactly once in the approved legacy cutover selects `HISTORICAL_PROMOTION_PARTIAL`. That historical path requires PRESENT `legacyPromotionCutoverReference` and NOT_AVAILABLE Plan, lineage, Candidate-transition, Receipt, baseline, and bootstrap-content references. An unlisted entry missing a valid Plan/binding/transition/Receipt is invalid rather than historical. Multiple matching Plans, bindings, or transitions, contradictory authorization markers, or any other authorization value are projection errors, never tie-broken guesses.
...
| Promotion path | Candidate integration transition | Candidate / Experiment / Trial / Eval references | Release expectation references | `releasedAt` |
| --- | --- | --- | --- | --- |
| `EXPERIMENT_BACKED` | exact PRESENT transition referenced by the terminal Receipt; pre-Candidate, transaction, command, Authority, applied entry, and `INTEGRATED` post-state all agree | Candidate is the binding's exact admitted `CandidateBundleReference` and its proposed content hash equals the canonical Capability; Experiment is its exact PRESENT reference; Trial references are the binding's sorted exact Trial array including the qualifying Trial; Eval Results are the sorted unique results reached from that binding's execution receipts | All and only valid frozen Expectations whose `ownerAnchor` exactly equals this `releaseId`; Experiment-owned Expectations are not silently copied | exact `ledgerEntry.authorizedAt`, which must be no later than transition `appliedAt` and Receipt `committedAt` |
| `CANDIDATE_EVAL_BACKED` | same exact PRESENT transition rule | Candidate is the binding's exact admitted bundle and its proposed content hash equals the canonical Capability; Experiment/Trial arrays are empty; Eval Results equal the binding's sorted exact legacy-result array and remain `LEGACY_ID_VERSION_ONLY` for content provenance | same owner-anchor rule | exact `ledgerEntry.authorizedAt`, no later than transition `appliedAt` and Receipt `committedAt` |
| `HISTORICAL_PROMOTION_PARTIAL` | `NOT_AVAILABLE: PRE_CUTOVER_NO_TRANSITION_CONTRACT` | all four arrays are empty; opaque source text and current files never fill historical lineage | same owner-anchor rule; absence is an empty known set only after the profile has completely scanned Release-owned Expectations | exact `ledgerEntry.authorizedAt` |
| `BOOTSTRAP_AUTHORIZED` | `NOT_AVAILABLE: BOOTSTRAP_HAS_NO_CANDIDATE_TRANSITION` | all four arrays are empty | same owner-anchor rule | exact `ledgerEntry.authorizedAt`, which must be at or before the baseline cutoff |
...
PROMOTION_PLAN
PROMOTION_JOURNAL
CANDIDATE_INTEGRATION_TRANSITION
EVAL_DEFINITION
EVAL_RESULT
GROWTH_EXPECTATION
GROWTH_EXPERIMENT
...
promotionPlans[]
promotionJournals[]
candidateIntegrationTransitions[]
experiments[]
trialAttempts[]
evalExecutionInputs[]
...
- Promotion Plans: `(transactionId, promotionPlanId, promotionPlanDigest)`;
- Promotion journals: `(transactionId, journalId, generation)` with entries ordered by positive sequence and generations linked by their previous-terminal-prefix reference;
- Candidate Integration Transitions: `(candidateId, transactionId, transitionId)`;
...
records {
  ...
  expectations, candidates, promotionPlans, promotionJournals,
  candidateIntegrationTransitions,
  experiments, trialAttempts,
...
The fixed `NO_GO` reasons are `BUILDER_PROFILE_INVALID | REQUIRED_PROVIDER_UNAVAILABLE | REQUIRED_SOURCE_UNKNOWN | REQUIRED_SOURCE_OMITTED | SOURCE_IDENTITY_CONFLICT | SOURCE_SCHEMA_INVALID | SOURCE_CUTOFF_INVALID | COHORT_IDENTITY_MISMATCH | CROSS_REFERENCE_INVALID | DIGEST_INVALID | PROMOTION_RECOVERY_REQUIRED | DERIVATION_INVARIANT_VIOLATION`. Gate is `NO_GO` exactly when the profile is invalid or unresolved, provider state is `UNAVAILABLE` or `UNKNOWN`, a required source is omitted, `invalidSourceCount > 0`, a post-cutover Candidate/Plan/Promotion-journal/ledger/binding/transition is awaiting Promotion recovery, or any listed identity, Schema, cutoff, cohort, cross-reference, digest, count, or derivation invariant fails. Otherwise it is `PASS`.
...
-> freeze one transaction-wide Promotion Plan
-> stage and durably validate canonical capability + promotion ledger + Promotion Lineage Binding
-> freeze apply intent and CAS Candidate AUTHORIZED -> INTEGRATED
-> persist Candidate Integration Transition
...
A PASSED Experiment establishes only its own outcome, not Promotion authority. `EXPERIMENT_BACKED` additionally requires exact same-Trial Eval receipts, a Promotion Lineage Intent consumed by the authorized Promotion command, one immutable Plan shared by every successful-generation event/artifact, a durable lineage binding, the Candidate status apply-intent/CAS/applied transition, its terminal Receipt published last, the canonical Capability, and the matching ledger entry. For modern Promotion, `ledgerEntry.authorizedAt <= candidateTransition.appliedAt <= promotionReceipt.committedAt`; references, Plan equality, and journal sequence, not timestamps alone, prove the causal order. Only a cutover-listed historical Promotion remains projectable as `HISTORICAL_PROMOTION_PARTIAL`; only an exact baseline-and-ledger-backed seed is `BOOTSTRAP_AUTHORIZED`.
...
CANDIDATE_INTEGRATED_LABEL != PROMOTION_COMMITTED
PROMOTION_COMMITTED REQUIRES CANDIDATE_INTEGRATION_TRANSITION
PROMOTION_RECOVERY_GENERATIONS REQUIRE SAME PLAN
...
| Candidate status CAS conflict or Authority changed before apply intent | Publish no applied entry or terminal Receipt; return conflict/uncertain outcome as applicable and keep staged artifacts non-authoritative |
| Promotion recovery changes Plan, forks the generation chain, or consumes staged/intent evidence from another generation | Reject the recovery chain, publish no terminal Receipt or Release, and set projection Gate `NO_GO` with `PROMOTION_RECOVERY_REQUIRED` or the more specific identity/digest reason |
| Candidate is `INTEGRATED` but transition or terminal Receipt is missing | Show `INTEGRATION_PENDING_RECEIPT | RECOVERY_REQUIRED`; emit no Release and never infer success from the mutable field |
| Candidate status interruption yields neither exact preimage nor planned postimage | Return `PROMOTION_OUTCOME_UNCERTAIN`; require explicit recovery and never auto-rollback, delete evidence, or publish a Receipt |
...
| Post-cutover Promotion lacks Plan/binding/Candidate transition/Receipt | Do not downgrade to historical partial; emit no Release and set Gate `NO_GO` |
...
3. coverage-aware lineage from Source -> Receipt -> Experience -> Candidate -> optional Experiment/Trial -> frozen Eval input -> ordered acceptance/start/completion/final-admission journal -> Eval Execution Receipt -> immutable Promotion Plan -> same-Plan nonbranching Promotion journal generations -> Candidate status CAS/Integration Transition -> human Promotion Receipt/ledger -> derived Release -> same-cohort Adoption -> Effect, with every allowed promotion path and every missing link explicit;
...
- Under an explicit fixed-candidate authority decision, freeze the one-time Legacy Promotion Cutover before enabling the new Promotion path; post-cutover missing Plan, lineage, Candidate transition, or Receipt never degrades to historical partial.
- Extend the existing direct Harness Promotion command so an Experiment-backed path consumes a frozen lineage intent, freezes one transaction-wide Promotion Plan, stages and validates the canonical Capability, ledger entry, and lineage binding against it, performs the exact same-Plan Candidate status apply-intent/CAS/applied transition in the same durable journal generation, then publishes the terminal Promotion Receipt last. This does not expose Promotion through Workbench.
...
- the immutable Promotion Plan and one nonbranching journal-generation chain proving every staged/applied artifact used that same Plan;
- the matching Candidate Integration Transition proving pre-Receipt `AUTHORIZED -> INTEGRATED` CAS and all interruption/recovery branches;
