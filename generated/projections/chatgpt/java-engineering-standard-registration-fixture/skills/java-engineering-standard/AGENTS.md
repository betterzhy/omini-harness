...
- `CANONICAL_CAPABILITY_ID=framework:java:java-engineering-standard` 由 omini-harness 分配，只表示本 Pack 的稳定能力身份。
- `REGISTRATION_STATUS=REGISTERED` 只描述本 Pack 已进入 omini-harness Registry；它不建立下游采用、安装、运行时投影、任务完成或发布授权。
