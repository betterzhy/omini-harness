...
The project directory, Pack machine name, and callable Skill name use `java-engineering-standard`. `UNASSIGNED` and `NOT_REGISTERED` are fail-closed source-project facts: they do not reserve or register an omini Capability Id and do not establish adoption, installation, or runtime projection. The string `java-spec` remains only as a 0.4.0 compatibility protocol namespace for Schema `$id` values, Adapter identities, `target/java-spec-quality/**`, `JAVA_SPEC_*` CI variables, and historical material.
...
- may later own capability source resolution, selection, immutable lock, resolved content digest, validator identity, and cross-project adoption governance;
- is not modified or integrated by this Slice. java-engineering-standard does not maintain a second source/adoption lock.
...
`JAVA_QUALITY_RESULT=VALID` means that the supplied, hash-bound Evidence is structurally and semantically valid under this Pack. It is not a cryptographic execution attestation: the target project remains the owner of workflow execution, raw outputs, candidate evidence and provenance authenticity. Future validator identity or signed cross-project attestation belongs to the caller/omini control plane, not to java-engineering-standard 0.4.0.
...
No Java, Spring, Maven, ArchUnit, or quality-tool upgrade; no Generated Source enablement; no Baseline debt acceptance; no global Skill install; no real downstream Pilot; no reusable workflow publication; no omini-harness, Pay-Nexus, database, network, deployment, push, release, or automatic main merge.
